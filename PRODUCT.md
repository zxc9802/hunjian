...
把中文文案交给既有 NAS 混剪服务，从真实素材匹配画面，生成配音与字幕，可选用户上传的背景音乐，并展示检查通过的成片。
...
- 原素材裁切，保持既有色彩处理；背景音乐仅使用用户上传的曲目，支持逐首试听，不选音乐时保留口播；Gemini 3.7 Flash 检查实际成片和声音。
- 制作任务保存在 NAS，关闭网页或电脑不停止任务。网页必须展示真实日志，不能伪造进度或通过结果。
- 第一版是单工作区；公网使用登录保护，NAS 与模型密钥不交给浏览器。NAS 已挂载素材库中的新视频会自动入库；目前没有多租户、网页素材上传或独立原素材搜索 API。
