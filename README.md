...
输入文案，从已有视频素材库匹配画面，生成配音与字幕，可选用户上传的背景音乐，检查成片后播放和下载。提供制作工作台、NAS 任务 API 和可复用的素材匹配技能。
...
制作中若语义匹配镜头不足，会从索引里补充动态画面并在计划里标记替代镜头；不会截短口播。失败或中断的任务保留阶段文件，可从工作台原任务继续。背景音乐仅使用用户上传到腾讯 COS 音乐库的曲目，每首支持试听、暂停和拖动进度；新任务选曲后会铺满并裁到视频结束。不上传或不选曲也可制作视频，保留口播，不添加背景音乐。
...
目前是单工作区、共享密码的制作工作台，使用既定音色与竖屏 1080p 默认设置，支持独立任务进度、封面候选、整片固定标题即时预览和下载时导出。NAS 单任务排队制作，切换或新建任务不会中断旧任务。尚未提供多租户隔离、在线更换音色、独立素材搜索和上传素材自动索引。
