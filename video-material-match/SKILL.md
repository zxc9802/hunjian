...
4. 三个都不相符时排除已拒绝片段，细化同义查询并扩大召回至 60、180，分别重新精排和观看。最多三轮；仍不符要明确标记该段缺少语义匹配素材。
5. 配音长于核验合格镜头时，先继续召回、精排并观看补充镜头。时长仍不足时，用户已允许按相关度使用其他动态空镜，必要时重复动态片段；不得只因语义素材不足使任务失败。将替代情况记为 `fallback-b-roll`，不冒充真实拍到文案设施。用动态镜头覆盖整句，不靠长定格、黑屏或截短口播补时。`cuts.json` 记录实际裁切区间。
...
- 优先补充或替换合格动态素材；不足时启用已授权的动态补充画面，不为凑镜头擅自改写文案。若另外获准修改文案，保留原文及逐段修改记录，含义、数字、事实承诺保持不变。修改后重新 `voice` 测量实际时长，再 `render` 与 Gemini 复查。
