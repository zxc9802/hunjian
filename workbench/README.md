...
已完成、失败、中断或被拒绝的记录，在工作台记录该结束状态后保留 72 小时。启动、健康检查及读取任务记录时自动清理到期记录；刷新状态不会延长保留时间，续作后重新计时。排队、运行及等待核对的任务继续保留。清理只删除工作台记录，不删除 NAS 素材、成片、音色文件或 COS 对象；COS 成片需要另行设置桶生命周期或人工清理。
...
- NAS 质检并上传私有腾讯 COS 成功后才标记视频 `done`。工作台按 SHA256 校验并缓存 COS 成片，播放和下载支持 Range，允许拖动进度条；查看检查报告可追溯实际导出文件的 SHA256。
